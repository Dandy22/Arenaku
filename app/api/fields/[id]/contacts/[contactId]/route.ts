...
export async function PATCH(
  ...
  try {
    ...
    const contact = await fieldService.updateContact(user.userId, user.role, contactId, {
      name: body.name,
      email: body.email,
      phone: body.phone,
    });
    ...
    if (error.message.includes("Only vendors") || error.message.includes("not authorized")) return NextResponse.json({ error: error.message }, { status: 403 });
