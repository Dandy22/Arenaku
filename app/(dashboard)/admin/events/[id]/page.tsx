...
export default function EventDynamicPage({
  ...
  const handleSaveDraft = async () => {
    ...
    try {
      ...
      if (isNewEvent) {
        const resEvent = await api.post("/admin/events", eventPayload);
      ...
      const errorMessage =
        err.response?.data?.error || err.message || "Gagal menyimpan draf";
  ...
  const handleCreateEvent = async () => {
    ...
    // FIX DI SINI: Biar dia tau kalau udah ada tiket dari database
    const currentTickets = isNewEvent ? localTickets : tickets;
    ...
    try {
      ...
      const eventPayload = {
        ...
        endDate:
          eventForm.date?.[1]?.toISOString?.() || new Date().toISOString(),
        status: "ACTIVE",
      };
      ...
      message.success("Event berhasil disimpan!");
      router.push(`/admin/events`);
    } catch (err: any) {
  ...
  const handleDeleteLocalTicket = (index: number) => {
    setLocalTickets(localTickets.filter((_, i) => i !== index));
  };

  const handleLiveTicketSubmit = async () => {
    if (!ticketForm.name || ticketForm.stock <= 0) {
      message.error("Mohon lengkapi Nama dan Jumlah tiket");
      return;
    }
    ...
    try {
      if (editingId) {
        ...
        await api.post(`/events/${eventId}/tickets`, finalTicket);
        message.success("Tiket ditambahkan");
      }
      setModalOpen(false);
      resetTicketForm();
      fetchTickets(eventId); // REFRESH DATA DARI DB
    } catch (err: any) {
  ...
  const handleDeleteLiveTicket = async (ticketId: string) => {
    Modal.confirm({
      title: "Hapus Tiket?",
      content: "Apakah Anda yakin ingin menghapus tiket ini?",
      okText: "Ya, Hapus",
      cancelText: "Batal",
      ...
      onOk: async () => {
        try {
          ...
          message.error(err.response?.data?.error || "Gagal menghapus tiket");
        }
      },
    });
  };
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <DatePicker.RangePicker
                      className="!w-full !rounded-lg !py-2.5 !border-gray-200 text-sm text-slate-500 font-semibold"
                    ...
                    <div className="border-t border-dashed border-gray-200 pt-3 flex items-center justify-between">
                      ...
                      <div className="flex gap-3 text-xs font-semibold">
                        ...
                        <button
                          onClick={() => {
                            if (isNewEvent) {
                              handleDeleteLocalTicket(index);
                            } else {
                              handleDeleteLiveTicket(ticket.id!);
                            }
                          }}
        ...
        <div className="pt-2 space-y-5">
          ...
          <div className="flex gap-4 mt-8 pt-6 border-t border-dashed border-gray-200">
            ...
            <Button
              ...
              onClick={
                isNewEvent ? handleLocalTicketSubmit : handleLiveTicketSubmit
              }
      ...
      <DeleteModal
        ...
        onDelete={() => {
          if (deleteTicketModal.ticketId) {
            handleDeleteLiveTicket(deleteTicketModal.ticketId);
          }
        }}
      />
